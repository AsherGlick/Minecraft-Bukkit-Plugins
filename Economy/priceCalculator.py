        ...
        for item in i:
            if isinstance(i[item], list):
                if item in multivalue:
                    for (index,price) in enumerate(i[item]):
                        if price != -1:
                            print '{{include("economy_templates/item.html", item="'+(item+'_'+multivalue[item][index]+'",').replace("_"," ").title().ljust(maxlen+2)+' price='+str(price).ljust(5)+')}}'
            else:
                if i[item] != -1:
                    print '{{include("economy_templates/item.html", item="'+(item+'",').replace("_"," ").title().ljust(maxlen+2)+' price='+str(int(i[item])).ljust(5)+')}}'
